        ...
        ${errorEventEntries
          ...
          <div style="background: white; padding: 15px; border-radius: 6px; margin-bottom: 15px; border-left: 4px solid ${logs[0].level === "ERROR" ? "#dc3545" : "#f59e0b"};">
            <h3 style="margin: 0 0 10px 0; font-size: 16px; color: ${logs[0].level === "ERROR" ? "#dc3545" : "#f59e0b"};">
              <span style="display: inline-block; min-width: 42px; font-size: 11px; font-weight: 700; color: white; background: ${logs[0].level === "ERROR" ? "#dc3545" : "#f59e0b"}; padding: 2px 6px; border-radius: 4px; text-align: center;">${logs[0].level === "ERROR" ? "ERROR" : "WARN"}</span>
              ${event}
...
Check the Logs sheet in Google Sheets for complete details.
  ...
  await transporter.sendMail({
    ...
    subject: "[RED ALERT] 6301926 ERROR LOGS",
    html: html,
    text: text,
  })
}
