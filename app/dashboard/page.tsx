...
// Import the export utilities at the top of the file
...
const translations = {
  ...
  he: {
    ...
    ofRecords: "מתוך",
    records: "רשומות",
    clickToViewNotCleared: "לחץ לצפייה בכל העסקאות שלא נוקו",
    showingNotClearedOnly: "מציג רק עסקאות שלא נוקו",
...
const translateNotes = (description: string, language: "en" | "he") => {
  ...
  // Comprehensive transaction descriptions translations for Hebrew
...
export default function Dashboard() {
  ...
  // Export function for all transactions
  const exportAllTransactions = () => {
    if (filteredTransactions.length > 0) {
      const header = ["Date", "Type", "Description", "Amount", "Net", "Status", "Cardknox"]
      const rows = filteredTransactions.map((tx) => [
        shouldHideDonationInfo(tx.donorName || "", "date") ? "***" : tx.date,
        translateType(tx.type, t),
        tx.description, // Notes are already in description for donations
        shouldHideDonationInfo(tx.donorName || "", "amount") ? "***" : tx.amount.toFixed(2),
        shouldHideDonationInfo(tx.donorName || "", "amount") ? "***" : tx.net.toFixed(2),
        tx.notCleared || "",
        tx.cardknox || "",
      ])

      const csv = [header, ...rows].map((row) => row.join(",")).join("\n")
      downloadCSV(csv, "all-transactions.csv")
    }
  }
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button
              ...
              className="border-white text-white hover:text-black transition-colors min-w-[90px] h-10"
              style={{
                background: "linear-gradient(135deg, #FFD700 0%, #FFA500 100%)",
                borderColor: "#FFD700",
              }}
            ...
            <CardHeader className="flex flex-row items-center justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  className="text-white font-semibold shadow-lg hover:shadow-xl transition-all duration-200 border-0"
                  style={{
                    background: "linear-gradient(135deg, #20B2AA 0%, #48D1CC 100%)",
                  }}
                >
